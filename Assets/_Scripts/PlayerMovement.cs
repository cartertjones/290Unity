...
public class PlayerMovement : MonoBehaviour {
    ...
    private void Movement()
    {
        ...
        // If speed is larger than max speed, cancel out the input so you don't go over max speed
        ...
        if (!grounded)
        {
         ...
         // Movement while sliding/crouching
        if (grounded && crouching)
        {
            if (sliding)
            {
                multiplierV = 0f;
                currentMaxSpeed = maxSpeed * 0.2f; // Adjust the sliding speed if needed
            }
            else
            {
                multiplierV = 1f;
    ...
    bool validGroundCollision = false;

    for (int i = 0; i < other.contactCount; i++) {
        Vector3 normal = other.contacts[i].normal;

        if (IsFloor(normal)) {
            // Check if the normal is mostly vertical, indicating ground
            grounded = true;
    ...
    // If no valid ground collision is found, consider the player not grounded
    if (!validGroundCollision) {
        grounded = false;
    }
